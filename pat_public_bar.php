...
function pat_public_bar($atts) {
	...
	if ( cs('txp_login_public') ) {
		...
		*/
		...
		$article = '<li>'.(
		$thisarticle['thisid'] ? 
		$add.' / <a href="'.$interface.'/index.php?event=article&amp;step=edit&amp;ID='.$thisarticle['thisid'].'&amp;_txp_token='.form_token().'">'.gTxt('edit').'  ('.$pretext['s'].')</a>' : 
		$add 
		).'</li>';
